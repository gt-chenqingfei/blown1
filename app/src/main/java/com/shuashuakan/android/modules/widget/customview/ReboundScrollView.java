...
public class ReboundScrollView extends LinearLayout {
  public static final int OVERSCROLL_DISTANCE = 400;
...
//        }
        ...
        float slopX = Math.abs(ev.getX() - firstX);
        float slopY = Math.abs(ev.getY() - firstY);
      ...
      case MotionEvent.ACTION_UP: {
        ...
        if (mOnReboundEndListener != null){
          if (this.getScrollY() < 0){
            mOnReboundEndListener.onReboundTopComplete(this.getScrollY());
          }
          if (this.getScrollY() > 0){
            mOnReboundEndListener.onReboundBottomComplete(this.getScrollY());
          }
        }
        break;
      }
  ...
  public interface OnReboundEndListener{
    ...
    void onReboundTopComplete(int distance);

    void onReboundBottomComplete(int distance);
  }
}