...
public class FlikerProgressBar extends View implements Runnable{
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        try {
            ...
            loadingColor = ta.getColor(R.styleable.FlikerProgressBar_loadingColor, ContextCompat.getColor(getContext(),R.color.color_normal_40c4ff));
            stopColor = ta.getColor(R.styleable.FlikerProgressBar_stopColor, ContextCompat.getColor(getContext(),R.color.color_normal_ff9800));
            radius = (int) ta.getDimension(R.styleable.FlikerProgressBar_flikeRadius, 0);
            borderWidth = (int) ta.getDimension(R.styleable.FlikerProgressBar_flikwBorderWidth, 1);
        } finally {
            ta.recycle();
        }
    }
    ...
    private String getProgressText() {
        ...
        if(!isFinish){
            if(!isStop){
                //text = "下载中" + progress + "%";
                text = getResources().getString(R.string.string_upload_wait);
            } else {
                text = getResources().getString(R.string.string_resume_label);
            }
        } else{
            //text = "下载完成";
            text = getResources().getString(R.string.string_solitaire);
        }
